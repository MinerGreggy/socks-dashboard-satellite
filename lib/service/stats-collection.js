...
class StatsCollection {
  ...
  async init() {
    ...
    this.origin = 'socks-dashboard-satellite';
    ...
    this.connection.addService(constants.SERVICE('socks').walletUi);
  ...
  async updateRunningServices() {
    ...
    await Promise.all(this.enabledServices.filter(service => service !== plotterService).map(async service => {
      let isRunning = await this.daemonApiClient.isServiceRunning(constants.SERVICE('socks')[service]);
